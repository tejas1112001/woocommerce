...
const Payment = ({
  ...
  const handleSubmit = async (paymentMethodId: string) => {
    ...
    try {
      ...
      // Refresh server components so CheckoutSummary's PaymentButton receives
      // the updated cart (with the newly created payment session). Without this,
      // PaymentButton reads the stale initial cart (no session) and stays disabled
