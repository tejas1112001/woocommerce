...
const Shipping: React.FC<ShippingProps> = ({
  ...
  const isDeliveryCompleted = (cart?.shipping_methods?.length ?? 0) > 0

  const isOpen =
    isAddressCompleted &&
    (activeStep === 'delivery' || !isDeliveryCompleted || !activeStep)
  ...
  const persistShippingMethod = async (id: string) => {
    setError(null)
    setIsLoading(true)
    ...
    try {
      ...
      setError(err?.message || 'Unable to select delivery option.')
      throw err
    } finally {
      setIsLoading(false)
    }
  ...
  const set = async (id: string) => {
    ...
    await persistShippingMethod(id)
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const currentShippingMethodId = cart.shipping_methods?.[0]?.shipping_option_id

      if (
        selectedShippingMethodId &&
        currentShippingMethodId !== selectedShippingMethodId
      ) {
        await persistShippingMethod(selectedShippingMethodId)
      }
    ...
    <Box className="bg-primary p-5">
      ...
      {isOpen ? (
        <Box data-testid="delivery-options-container">
          <RadioGroup value={selectedShippingMethodId} onChange={set}>
