...
const RenderModernMegaMenuContent = ({
  itemsList,
  activeItem,
  item,
  onOpenChange,
  ...
  return (
    <div className="mx-auto w-full max-w-[1360px] px-6 py-6 medium:px-12">
      {/* ── Header ─────────────────────────────────────────── */}
      <div className="mb-5 flex flex-col gap-3 medium:flex-row medium:items-center medium:justify-between">
        <div>
          <h2 className="text-xl font-bold tracking-tight text-[#0A0A0A] medium:text-2xl flex items-center gap-2.5">
            <span>{title}</span>
            <span className="h-1.5 w-1.5 rounded-full bg-[#D4AF37]" />
          </h2>
          <p className="mt-0.5 text-xs text-gray-500 medium:text-sm">
            {subtitle}
          </p>
        </div>
      ...
      {/* Thin theme divider */}
      <div className="mb-5 h-px bg-gradient-to-r from-transparent via-[#D4AF37]/30 to-transparent" />

      {/* ── Content Grids ──────────────────────────────────── */}
      {isCollectionsView ? (
        /* ── Collections Grid (Compact Cards, scales for 6–12 items) ── */
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {itemsList.map((collection, index) => {
            const isCardActive = collection.handle ? pathname.includes(collection.handle) : false
            return (
              <LocalizedClientLink
                key={index}
                ...
                onClick={() => onOpenChange(false)}
                className={cn(
                  'group relative flex items-center gap-3.5 rounded-xl p-3.5 shadow-2xs transition-all duration-200 border',
                  isCardActive
                    ? 'border-[#D4AF37] bg-[#6B0014]/5 shadow-sm'
                    : 'border-gray-200/70 bg-white hover:-translate-y-0.5 hover:border-[#D4AF37]/60 hover:shadow-md'
                )}
              ...
              >
                ...
                {/* Icon badge */}
                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-[#6B0014]/10 to-[#D4AF37]/20 border border-[#D4AF37]/30 text-[#6B0014] transition-all duration-200 group-hover:bg-[#6B0014] group-hover:text-[#D4AF37]">
                  <svg
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
          {itemsList.map((category, index) => {
            ...
            const isCardActive = category.handle ? pathname.includes(category.handle) : false
            return (
              <LocalizedClientLink
                key={index}
                ...
                onClick={() => onOpenChange(false)}
                className={cn(
                  'group relative flex flex-col overflow-hidden rounded-xl p-3.5 shadow-2xs transition-all duration-200 border min-h-[120px]',
                  isCardActive
                    ? 'border-[#D4AF37] bg-[#6B0014]/5 shadow-sm'
                    : 'border-gray-200/70 bg-white hover:-translate-y-0.5 hover:border-[#D4AF37]/60 hover:shadow-md'
                )}
              ...
              >
                ...
                {/* Top row: Icon badge */}
                <div className="relative mb-2.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gray-100/80 text-[#6B0014] transition-all duration-200 group-hover:bg-[#6B0014] group-hover:text-[#D4AF37]">
                  <svg
                ...
                {/* Name */}
                <h3 className="relative mb-1 text-sm font-bold leading-snug text-[#0A0A0A] transition-colors duration-200 group-hover:text-[#6B0014]">
                  {category.name}
                </h3>
                ...
                {category.category_children && category.category_children.length > 0 && (
                  <div className="relative mt-1 space-y-0.5">
                    {category.category_children.slice(0, 2).map((subcat, subIndex) => (
                      <div
                        key={subIndex}
                        className="flex items-center gap-1 text-[11px] text-gray-500 transition-colors duration-200 group-hover:text-gray-700"
                      >
                    ...
                    {category.category_children.length > 2 && (
                      <div className="text-[10px] font-semibold text-[#6B0014] group-hover:text-[#D4AF37]">
                        +{category.category_children.length - 2} more
                      </div>
                    )}
                  </div>
                )}
                ...
                <div className="relative mt-auto flex items-center gap-1 pt-2.5 text-xs font-semibold text-gray-400 transition-colors duration-200 group-hover:text-[#D4AF37]">
                  <span>Shop</span>
                  <svg
                    className="h-3 w-3 transition-transform duration-200 group-hover:translate-x-0.5"
                    fill="none"
      ...
      {/* ── Bottom CTA Banner ── */}
      <div className="relative mt-5 overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-r from-[#0A0A0A] via-[#580010] to-[#2B0008] p-5 text-white shadow-xl">
        ...
        <div className="relative flex flex-col items-start justify-between gap-4 medium:flex-row medium:items-center">
          <div>
            <h3 className="flex items-center gap-2 text-sm font-bold medium:text-base">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <LocalizedClientLink
              href="/contact"
              onClick={() => onOpenChange(false)}
              className="inline-flex items-center gap-2 rounded-xl bg-[#D4AF37] px-4 py-2 text-xs font-semibold text-[#0A0A0A] shadow-md transition-all duration-200 hover:bg-[#c59b27] hover:shadow-lg"
            >
            ...
            <LocalizedClientLink
              href="/about"
              onClick={() => onOpenChange(false)}
              className="inline-flex items-center gap-2 rounded-xl border border-[#D4AF37]/40 bg-white/5 px-4 py-2 text-xs font-semibold text-white backdrop-blur-xs transition-all duration-200 hover:border-[#D4AF37] hover:bg-white/10"
            >
...
const DropdownMenu: React.FC<DropdownMenuProps> = ({
  ...
  const handleMouseLeave = () => {
    clearCloseTimeout()
    // 250ms stable grace period to prevent accidental flickering when moving cursor
    timeoutRef.current = setTimeout(() => {
      onOpenChange(false)
    }, 250)
  }

  // Force close on scroll - prevents stuck menu when user scrolls
  ...
  // Outside click / touch handling: close mega menu when clicking anywhere outside
  useEffect(() => {
      ...
      {item.category_children && item.category_children.length > 0 && (
        <>
          ...
          <div
            ...
            onMouseEnter={() => {
              // When cursor moves to backdrop, immediately close
              clearCloseTimeout()
              onOpenChange(false)
            }}
            className={cn(
              'fixed inset-0 top-[88px] z-30 bg-black/35 backdrop-blur-2xs transition-opacity duration-200',
              isOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
            )}
          />

          {/* Fixed Mega Menu Panel */}
          <div
            ref={panelRef}
            onMouseEnter={handleMouseEnter}
            onMouseLeave={handleMouseLeave}
            ...
            style={{
              top: '88px',
              maxHeight: isOpen ? 'calc(85vh - 88px)' : '0',
          ...
          >
            ...
            {customContent ?? (
              <RenderModernMegaMenuContent
                itemsList={item.category_children}
